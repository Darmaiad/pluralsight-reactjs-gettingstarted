...
export default class Header extends React.Component {
      ...
      this.setState(prevState => ({
        timerButton: (
          <Button
            ...
            disabled={true}
          >
            {prevState.seconds > 10
              ? '0:' + prevState.seconds
              : '0:0' + prevState.seconds}
          </Button>
        ),
        limbo: true
      }));
  ...
  handleTimer = () => {
    alertify.success(
      '<strong>Game Started <i class="fa fa-play"></i></strong>',
      this.state.alertWaitingTimeSecs
    );
    this.setState(
      prevState => ({
  ...
  handleSetTimeOut = () => {
    alertify.error(
      '<strong>Time Ran Out <i class="fa fa-stop"></i></strong>',
      this.state.alertWaitingTimeSecs
    );
    ...
    return (
      <div>
        <Row className="header-row">
          <Col xs={12} md={6}>
            <h3 className="custom-header">Play Nine9</h3>
          </Col>
          <Col xs={12} md={6}>
